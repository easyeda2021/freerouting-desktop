...
export default function MenuBar() {
  ...
  const handleOpenDsn = async () => {
    try {
      ...
      streamLogs(job.id, (log) => {
        ...
        // Extract score from log messages
        const match = log.message.match(/score of ([\d.]+)/i)
        if (match) {
          dispatch({ type: 'SET_SCORE', score: parseFloat(match[1]) })
        }
      })

      // Start output stream for real-time SES updates
      ...
      const poll = setInterval(async () => {
        try {
          ...
          dispatch({
            ...
            state: status.state,
            stage: status.stage || '',
            currentPass: status.current_pass || 0,
          })
      ...
      dispatch({
        type: 'ADD_LOG',
        entry: { timestamp: new Date().toISOString(), type: 'Error', message: String(err), topic: 'App' },
      })
    }
  }
  ...
  const handleExportSes = async () => {
    ...
    try {
      ...
      if (path) {
        window.writeFile(path, sesContent)
      }
    } catch (err) {
      dispatch({
        type: 'ADD_LOG',
  ...
  return (
    <div style={s.bar}>
      ...
      <div style={s.menu}>
        <button style={s.btn} onClick={handleOpenDsn} disabled={state.jarStatus !== 'ready'}>
          Open DSN
        </button>
        ...
        <span style={s.status}>
          JAR: {state.jarStatus === 'ready' ? `v${state.jarVersion || '?'}` : state.jarStatus}
        </span>
      </div>
    </div>
  )
